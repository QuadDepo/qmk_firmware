...
lp50xx_result_e
lp50xx_string_rgb_set32(lp50xx_t* driver, uint8_t start_led, uint8_t length, uint32_t rgb_color)
{
    if (driver->state != LP50XX_STATE_NORMAL)
        return LP50XX_RESULT_INVALID_STATE;
    ...
    uint8_t reg = driver->reg_info->out_color + 3 * start_led;

    uint8_t red = (rgb_color >> 16) & 0xff;
    uint8_t green = (rgb_color >> 8) & 0xff;
    uint8_t blue = rgb_color & 0xff;
